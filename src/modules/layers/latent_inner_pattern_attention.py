...
class ScaledDotProductLatentInnerPatternAttention(nn.Module):
...
class MultiHeadLatentInnerPatternAttention(nn.Module):
    def __init__(self, emb_dim, n_heads, rnn_hidden_dim, dropout_attn=0.0, dropout_attn_out=0.0):
        ...
        self.attention = ScaledDotProductLatentInnerPatternAttention(temperature=emb_dim ** 0.5, dropout_attn=dropout_attn)
    ...
    def set_cmi(self, use_cmi):
        self.disentangle_x = []
        self.cmi_attn_select = []
        self.cmi_attn_latent = []
        self.use_cmi = use_cmi
    ...
    def cal_disentangle_loss(self, mode="contrastive"):
        x = torch.cat(self.disentangle_x, dim=0)  # b * t, h, e (b = s * b)
        loss = None
        if mode == "contrastive":
            dist = torch.bmm(x, x.permute(0, 2, 1))  # b * t, h, h
            positive_res = torch.exp(torch.diagonal(dist, dim1=-2, dim2=-1) - torch.max(dist, dim=2)[0])  # b * t, h
            negative_res = torch.sum(torch.exp(dist - torch.max(dist, dim=2)[0].unsqueeze(2)), dim=2)  # b * t, h
            loss = torch.mean(-torch.log(positive_res / negative_res), dim=1)  # b * t
        elif mode == "consistent":
            bt, h, e = x.size()
            labels = torch.ones((bt, h)) * torch.arange(h).unsqueeze(0)  # b * t, h
            labels = labels.long().to(x.device)
            pred = self.disentangle_classifier(x)  # b * t, h
            loss = torch.mean(self.disentangle_loss(pred, labels), dim=1)  # b * t
...
class LatentInnerPatternTransformerBlock(nn.Module):

    def __init__(self, emb_dim, n_heads, ff_emb_dim, rnn_hidden_dim, dropout_attn=0.0, dropout_attn_out=0.0, dropout_ff=0.0):
        super().__init__()

        self.attn = MultiHeadLatentInnerPatternAttention(emb_dim, n_heads, rnn_hidden_dim, dropout_attn, dropout_attn_out)
...
class LatentInnerPatternTransformer(nn.Module):
    ...
    def __init__(self, n_blocks, emb_dim, n_heads, ff_emb_dim, rnn_hidden_dim,
        ...
        self.transformer_blocks = nn.Sequential(*[
            LatentInnerPatternTransformerBlock(emb_dim, n_heads, ff_emb_dim, rnn_hidden_dim, dropout_attn, dropout_attn_out, dropout_ff)
