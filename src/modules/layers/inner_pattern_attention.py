...
class ScaledDotProductInnerPatternAttention(nn.Module):
...
class MultiHeadInnerPatternAttention(nn.Module):
    def __init__(self, emb_dim, n_heads, dropout_attn=0.0, dropout_attn_out=0.0):
        ...
        self.attention = ScaledDotProductInnerPatternAttention(temperature=emb_dim ** 0.5, dropout_attn=dropout_attn)
    ...
    def forward(self, x, mask=None):
        ...
        attn_select = x
        if mask is not None:
            attn_select = torch.bmm((~mask[:b]).float(), attn_select)
        attn_select = self.fc_select(attn_select)
        attn_select = F.softmax(attn_select, dim=2)
        attn_select = attn_select.view(b, t, 1, n_heads)
...
class InnerPatternTransformerBlock(nn.Module):

    def __init__(self, emb_dim, n_heads, ff_emb_dim, dropout_attn=0.0, dropout_attn_out=0.0, dropout_ff=0.0):
        super().__init__()

        self.attn = MultiHeadInnerPatternAttention(emb_dim, n_heads, dropout_attn, dropout_attn_out)
...
class InnerPatternTransformer(nn.Module):
    ...
    def __init__(self, n_blocks, emb_dim, n_heads, ff_emb_dim,
        ...
        self.transformer_blocks = nn.Sequential(*[
            InnerPatternTransformerBlock(emb_dim, n_heads, ff_emb_dim, dropout_attn, dropout_attn_out, dropout_ff)
