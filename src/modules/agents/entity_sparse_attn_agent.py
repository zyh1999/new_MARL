...
from modules.layers import MultiHeadSparseAttention


class EntitySparseAttnAgent(nn.Module):
    def __init__(self, input_shape, args):
        super(EntitySparseAttnAgent, self).__init__()
        self.args = args

        self.entity_embedding = nn.Linear(input_shape, args.emb_dim)
        self.attn = MultiHeadSparseAttention(args.emb_dim, args.n_heads)
