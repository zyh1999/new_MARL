...
from modules.layers import LatentInnerPatternTransformer


class EntityLatentInnerPatternTransAgent(nn.Module):
    def __init__(self, input_shape, args):
        super(EntityLatentInnerPatternTransAgent, self).__init__()
        self.args = args
        self.entity_shape = None

        self.entity_embedding = nn.Linear(input_shape, args.emb_dim)
        self.transformer = LatentInnerPatternTransformer(args.n_blocks, args.emb_dim, args.n_heads, args.emb_dim * 4, args.rnn_hidden_dim)
    ...
    def set_cmi(self, use_cmi):
        for block in self.transformer.transformer_blocks:
            block.attn.set_cmi(use_cmi=use_cmi)
