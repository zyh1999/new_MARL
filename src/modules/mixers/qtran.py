...
class QTranBase(nn.Module):
    def __init__(self, args):
        ...
        if self.arch == "coma_critic":
            ...
            if self.args.agent in ['updet', 'ext', 'latent']:
                q_input_size = self.state_dim + self.args.emb_dim + self.n_actions
            else:
        ...
        if self.args.network_size == "small":
            ...
            if self.args.agent not in ['updet', 'ext', 'latent']:
                ae_input = self.args.rnn_hidden_dim + self.n_actions
            else:
                ae_input = self.args.emb_dim + self.n_actions
