...
class TokenAttnQMixer(nn.Module):
    def __init__(self, args):
        super(TokenAttnQMixer, self).__init__()

        self.args = args

        self.token_embedding = nn.Linear(args.state_token_dim, args.mix_emb_dim)
        self.attn = MultiHeadAttention(args.mix_emb_dim, args.n_heads)
    ...
    def forward(self, agent_qs, states):
        ...
        x = self.attn.forward(x)[:, :self.args.n_agents]  # [b * s, n, e]
        ...
        q_tot = q_tot.view(b, s, 1)  # [batch_size, max_seq_length, 1]
