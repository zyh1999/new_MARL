...
from utils.statistics import statistic_q, statistic_attn
...
def run(_run, _config, _log):
    ...
    if args.use_tensorboard:
        logger.setup_tb(args.file_obs_path)
...
def evaluate_sequential(args, runner, learner):
    for _ in range(1):
        ...
        learner.evaluate(episode_batch)
    runner.close_env()
'''
...
def run_sequential(args, logger):
    ...
    args.action_ally_wise = env_info["action_ally_wise"]
    args.action_enemy_wise = env_info["action_enemy_wise"]
    ...
    if args.checkpoint_path != "":
        ...
        timestep_to_load = 0
        print("os=",os.getcwd())
        os.chdir("D:\\RL\\new_MARL")   # todo  need to change if others use
        print(os.getcwd())
        ...
        if args.statistic:
            statistic_attn(args, runner, learner)
            return
    ...
    while runner.t_env <= args.t_max:
        ...
        if args.save_model and (runner.t_env - model_save_time >= args.save_model_interval or model_save_time == 0):
            ...
            save_path = os.path.join(args.file_obs_path, "models", str(runner.t_env))
        ...
        if (runner.t_env - last_log_T) >= args.log_interval:
            logger.log_stat("run/episode", episode, runner.t_env)
