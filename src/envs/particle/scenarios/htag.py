import numpy as np
from envs.particle.core import World, Agent, Landmark, Action
...
class Scenario(BaseScenario):
    def make_world(self, args):
        ...
        num_landmarks = getattr(args, "num_landmarks", 2)
    ...
    def prey_policy(self, agent, world):
        ...
        if agent.dead:
            action.u = np.array([0.0, 0.0], dtype=np.float32)
            return action
    ...
    def get_done(self, world):
        ...
        done = all([a.dead for a in agents])
        return done
    ...
    def observation(self, agent, world):
        ...
        for other in agents:
            ...
            if not other.dead and (agent.view_radius == -1 or dist <= agent.view_radius):
                other_agent_pos.append(np.hstack([np.array([1.0]), other.state.p_pos, other.state.p_vel, np.array([0.0, 1.0, 0.0])]))
            else:
                other_agent_pos.append(np.array([0., 0., 0., 0., 0., 0., 0., 0.]))
        ...
        for other in adversaries:
            ...
            dist = np.sqrt(np.sum(np.square(other.state.p_pos - agent.state.p_pos)))
            if not other.dead and (agent.view_radius == -1 or dist <= agent.view_radius):
                other_adversary_pos.append(np.hstack([np.array([1.0]), other.state.p_pos, other.state.p_vel, np.array([1.0, 0.0, 0.0])]))
            else:
                other_adversary_pos.append(np.array([0., 0., 0., 0., 0., 0., 0., 0.]))

        entity_pos = []
        for entity in world.landmarks:
            dist = np.sqrt(np.sum(np.square(entity.state.p_pos - agent.state.p_pos)))
            if not entity.boundary and (agent.view_radius == -1 or dist <= agent.view_radius):
                entity_pos.append(np.hstack([np.array([1.0]), entity.state.p_pos, entity.state.p_vel, np.array([0.0, 0.0, 1.0])]))
            else:
                entity_pos.append(np.array([0., 0., 0., 0., 0., 0., 0., 0.]))

        return np.concatenate([np.array([1.0])] + [agent.state.p_pos] + [agent.state.p_vel] + [np.array([1.0, 0.0, 0.0])] + other_agent_pos + entity_pos + other_adversary_pos)
    ...
    def state(self, world):
        ...
        for other in world.agents:
            if not other.dead:
                if other.adversary:
                    other_pos.append(np.hstack([np.array([1.0]), other.state.p_pos, other.state.p_vel, np.array([1.0, 0.0, 0.0])]))
                else:
                    other_pos.append(np.hstack([np.array([1.0]), other.state.p_pos, other.state.p_vel, np.array([0.0, 1.0, 0.0])]))
            else:
