...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.n_actions_no_attack = 6  # no-op (valid only when dead), stop, move[direction] (four directions)
        self.n_actions_move = 4  # four directions: north, south, east, or west
        ...
        if self.action_grid_attack:
            ...
            # Actions: no-op, stop, move[direction], attack[enemy_id]
            self.n_actions = self.n_actions_no_attack + self.n_enemies

        self.state_align = state_align
        if self.state_align:
        ...
        self.obs_align = obs_align

        self.unit_type_align = unit_type_align
        if self.unit_type_align:
            if self.map_name in ["3s_vs_3z", "3s_vs_4z", "3s_vs_5z"]:
                self.unit_type_bits = 2
                self.map_type = "stalkers_and_zealots"
            elif self.map_name in ["2m_vs_1z"]:
                self.unit_type_bits = 2
                self.map_type = "marines_and_zealots"
            elif self.map_name in ["2s_vs_1sc"]:
                self.unit_type_bits = 2
                self.map_type = "stalkers_and_spinecrawlers"
            elif self.map_name in ["6h_vs_8z"]:
                self.unit_type_bits = 2
                self.map_type = "hydralisks_and_zealots"
            elif self.map_name in ["corridor"]:
                self.unit_type_bits = 2
                self.map_type = "zealots_and_zerglings"
            elif self.map_name in ["so_many_baneling"]:
                self.unit_type_bits = 2
                self.map_type = "zealots_and_banelings"
            elif self.map_name in ["2c_vs_64zg"]:
                self.unit_type_bits = 2
                self.map_type = "colossus_and_zerglings"
        ...
        if self.state_align:
            self.enemy_state_attr_names = ["health", "placeholder", "rel_x", "rel_y"]
        else:
        ...
        self.shield_align = shield_align
        if self.shield_align:
            self.shield_align_unit = None
            if self.shield_bits_ally != self.shield_bits_enemy:
                if self.shield_bits_ally == 0:
                    self.shield_align_unit = "ally"
                    self.ally_state_attr_names += ["placeholder_shield"]
                elif self.shield_bits_enemy == 0:
                    self.shield_align_unit = "enemy"
                    self.enemy_state_attr_names += ["placeholder_shield"]
        ...
        self.hydralisk_id = self.zergling_id = self.baneling_id = self.spinecrawler_id = 0
    ...
    def get_obs_agent(self, agent_id):
        ...
        if self.action_grid_attack and not self.obs_align:
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
                ...
                if (
                    ...
                    if self.obs_all_health:
                        ...
                        if self.shield_bits_enemy > 0:
                            ...
                            ind += 1
                        elif self.shield_align:
                            if self.shield_align_unit == "enemy":
                                enemy_feats[e_id, ind] = 0  # shield placeholder
                                ind += 1
                ...
                if (
                    ...
                    if self.obs_all_health:
                        ...
                        if self.shield_bits_ally > 0:
                            ...
                            if self.shield_align_unit == "ally":
                                ally_feats[i, ind] = 0  # shield placeholder
                                ind += 1
            ...
            if self.obs_own_health:
                ...
                if self.shield_bits_ally > 0:
                    ...
                    if self.shield_align_unit == "ally":
                        own_feats[ind] = 0  # shield placeholder
                        ind += 1
        ...
        if self.obs_align:
            ...
            #         own_feats.flatten(),
            #         np.concatenate((np.zeros((enemy_feats.shape[0], move_feats_dim), dtype=np.float32), enemy_feats), axis=1).flatten(),
            #         np.concatenate((np.zeros((ally_feats.shape[0], move_feats_dim), dtype=np.float32), ally_feats), axis=1).flatten(),
            #     )
            # )
        else:
            agent_obs = np.concatenate(
    ...
    def get_state_dict(self):
        ...
        for al_id, al_unit in self.agents.items():
            if al_unit.health > 0:
                ...
                if self.shield_bits_ally > 0:
                    ...
                    if self.shield_align_unit == "ally":
                        ally_state[al_id, 4] = 0  # shield placeholder
        ...
        for e_id, e_unit in self.enemies.items():
            if e_unit.health > 0:
                ...
                enemy_state[e_id, placeholder_bit + 1] = (
                                                                 x - center_x
                                                         ) / self.max_distance_x  # relative X
                enemy_state[e_id, placeholder_bit + 2] = (
                                                                 y - center_y
                                                         ) / self.max_distance_y  # relative Y

                if self.shield_bits_enemy > 0:
                    max_shield = self.unit_max_shield(e_unit)
                    enemy_state[e_id, placeholder_bit + 3] = e_unit.shield / max_shield  # shield
                elif self.shield_align:
                    if self.shield_align_unit == "enemy":
                        enemy_state[e_id, placeholder_bit + 3] = 0  # shield placeholder
    ...
    def get_obs_size(self):
        ...
        # if self.obs_align:
        #     return move_feats + 4 + own_feats + n_enemies * move_feats + enemy_feats + n_allies * move_feats + ally_feats
    ...
    def get_state_size(self):
        ...
        if self.shield_align:
            if self.shield_align_unit == "ally":
                nf_al += 1
            elif self.shield_align_unit == "enemy":
                nf_en += 1
    ...
    def get_unit_type_id(self, unit, ally):
        ...
        if ally:  # use new SC2 unit types
            ...
            if self.map_type == "stalkers_and_zealots":
                ...
                # id(Colossus) = 4, id(Stalker) = 74, id(Zealot) = 73
                if unit.unit_type == 4:
                    type_id = 0
                ...
                if unit.unit_type == 51:
                    ...
                    type_id = 2
            elif self.map_type == "marines_and_zealots":
                # id(Marine) = 48, id(Zealot) = 73
                if self.unit_type_align:
                    ...
                    assert False, 'unit type align error'
            elif self.map_type == "stalkers_and_spinecrawlers":
                # id(Stalker) = 74, id(Spine Crawler) = 98
                if self.unit_type_align:
                    if unit.unit_type == 74:
                        type_id = 0
                    else:
                    ...
                    assert False, 'unit type align error'
            elif self.map_type == "hydralisks_and_zealots":
                # id(Hydralisk) = 107, id(Zealot) = 73
                if self.unit_type_align:
                    ...
                    assert False, 'unit type align error'
            elif self.map_type == "zealots_and_zerglings":
                # id(Zealot) = 73, id(Zergling) = 105
                if self.unit_type_align:
                    ...
                    assert False, 'unit type align error'
            elif self.map_type == "zealots_and_banelings":
                # id(Zealot) = 73, id(Baneling) = 9
                if self.unit_type_align:
                    if unit.unit_type == 73:
                        type_id = 0
                    else:
                    ...
                    assert False, 'unit type align error'
            elif self.map_type == "colossus_and_zerglings":
                # id(Colossus) = 4, id(Zergling) = 105
                if self.unit_type_align:
                    if unit.unit_type == 4:
                        type_id = 0
                    else:
    ...
    def _init_ally_unit_types(self, min_unit_type):
        ...
        if self.map_type == "marines":
            ...
            self.stalker_id = min_unit_type
            self.spinecrawler_id = min_unit_type + 1
        elif self.map_type == "hydralisks_and_zealots":
            self.hydralisk_id = min_unit_type
            self.zealot_id = min_unit_type + 1
        elif self.map_type == "zealots_and_zerglings":
            ...
            self.zealot_id = min_unit_type
            self.baneling_id = min_unit_type + 1
        elif self.map_type == "colossus_and_zerglings":
            self.colossus_id = min_unit_type
            self.zergling_id = min_unit_type + 1
    ...
    def get_env_info(self):
        ...
        if self.obs_align and self.state_align:
            ...
            env_info["obs_mask_bit"] = 4  # 8 if pad zero move feature
