...
class StarCraft2Renderer:
    ...
    def draw_grid(self, surf):
        ...
        grid_unit_size = (shoot_range * 2.0) / action_grid_segment

        i = 0
        while i * grid_unit_size + self._playable.l < self._playable.b:
            surf.draw_line(colors.white, point.Point(i * grid_unit_size + self._playable.l, self._playable.t), point.Point(i * grid_unit_size + self._playable.l, self._playable.b))
            i += 1

        j = 0
        while j * grid_unit_size + self._playable.t < self._playable.r:
            surf.draw_line(colors.white, point.Point(self._playable.l, j * grid_unit_size + self._playable.t), point.Point(self._playable.r, j * grid_unit_size + self._playable.t))
            j += 1
        ...
        surf.write_screen(
            ...
            colors.white,
            (0, 0),
            "shoot_range: %s, grid_unit_size: %s" % (shoot_range, grid_unit_size,)
        )
