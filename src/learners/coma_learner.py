    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            for key in ["critic_loss", "critic_grad_norm", "td_error_abs", "q_taken_mean", "target_mean"]:
                self.logger.log_stat('train/' + key, sum(critic_train_stats[key])/ts_logged, t_env)

            self.logger.log_stat("train/advantage_mean", (advantages * mask).sum().item() / mask.sum().item(), t_env)
            self.logger.log_stat("train/coma_loss", coma_loss.item(), t_env)
            self.logger.log_stat("train/agent_grad_norm", grad_norm.item(), t_env)
            self.logger.log_stat("train/pi_max", (pi.max(dim=1)[0] * mask).sum().item() / mask.sum().item(), t_env)
