...
from modules.mixers.token_attn_qmix import TokenAttnQMixer
from modules.mixers.token_trans_qmix import TokenTransQMixer
from modules.mixers.token_pattern_trans_qmix import TokenPatternTransQMixer
import torch as th
import numpy as np
...
class QLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.mixer is not None:
            if args.mixer == "vdn":
                self.mixer = VDNMixer()
            elif args.mixer == "token_attn_qmix":
                self.mixer = TokenAttnQMixer(args)
            elif args.mixer == "token_trans_qmix":
                self.mixer = TokenTransQMixer(args)
            elif args.mixer == "token_pattern_trans_qmix":
                self.mixer = TokenPatternTransQMixer(args)
            else:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.args.agent in ["token_latent_pattern_wise_trans"]:
        ...
        loss = td_loss
        if self.args.mixer in ["token_pattern_trans_qmix"]:
        ...
        if self.args.agent in ["token_pattern_wise_trans", "token_latent_pattern_wise_trans"]:
