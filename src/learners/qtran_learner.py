...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("train/opt_loss", opt_loss.item(), t_env)
            self.logger.log_stat("train/nopt_loss", nopt_loss.item(), t_env)
            ...
            if self.args.mixer == "qtran_base":
                ...
                self.logger.log_stat("train/td_targets", ((masked_td_error).sum().item()/mask_elems), t_env)
                self.logger.log_stat("train/td_chosen_qs", (joint_qs.sum().item()/mask_elems), t_env)
                self.logger.log_stat("train/v_mean", (vs.sum().item()/mask_elems), t_env)
                self.logger.log_stat("train/agent_indiv_qs", ((chosen_action_qvals * mask).sum().item()/(mask_elems * self.args.n_agents)), t_env)
