...
from modules.mixers.entity_attn_qmix import EntityAttnQMixer
from modules.mixers.entity_multi_attn_qmix import EntityMultiAttnQMixer
from modules.mixers.entity_trans_qmix import EntityTransQMixer
from modules.mixers.entity_spotlight_trans_qmix import EntitySpotlightTransQMixer
from modules.mixers.entity_pattern_trans_qmix import EntityPatternTransQMixer
from modules.mixers.entity_inner_pattern_trans_qmix import EntityInnerPatternTransQMixer
from modules.mixers.entity_refil_qmix import FlexQMixer
import torch as th
...
class QLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.mixer is not None:
            if args.mixer == "vdn":
                self.mixer = VDNMixer()
            elif args.mixer == "entity_attn_qmix":
                self.mixer = EntityAttnQMixer(args)
            elif args.mixer == "entity_multi_attn_qmix":
                self.mixer = EntityMultiAttnQMixer(args)
            elif args.mixer == "entity_trans_qmix":
                self.mixer = EntityTransQMixer(args)
            elif args.mixer == "entity_spotlight_trans_qmix":
                self.mixer = EntitySpotlightTransQMixer(args)
            elif args.mixer == "entity_pattern_trans_qmix":
                self.mixer = EntityPatternTransQMixer(args)
            elif args.mixer == "entity_inner_pattern_trans_qmix":
                self.mixer = EntityInnerPatternTransQMixer(args)
            elif args.mixer == "entity_refil_qmix":
        ...
        params_size = get_params_size(self.params)
        self.logger.console_logger.info("params_size: {}".format(params_size))
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.args.agent in ["entity_latent_inner_pattern_trans"]:
            self.mac.agent.set_cmi(use_cmi=True)
        ...
        loss = td_loss
        if self.args.mixer in ["entity_spotlight_trans_qmix", "entity_pattern_trans_qmix", "entity_inner_pattern_trans_qmix"]:
            mixer_disentangle_loss = self.mixer.get_disentangle_loss(self.args.disentangle_mode)
        ...
        if self.args.agent in ["entity_spotlight_trans", "entity_pattern_trans", "entity_inner_pattern_trans", "entity_latent_inner_pattern_trans"]:
            mac_disentangle_loss = self.mac.agent.get_disentangle_loss(self.args.disentangle_mode)
        ...
        if self.args.agent in ["entity_latent_inner_pattern_trans"]:
            self.mac.agent.set_cmi(use_cmi=False)
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            if self.args.mixer in ["entity_spotlight_trans_qmix", "entity_pattern_trans_qmix", "entity_inner_pattern_trans_qmix"]:
                self.logger.log_stat("train/mixer_disentangle_loss", mixer_disentangle_loss.item(), t_env)
            if self.args.agent in ["entity_spotlight_trans", "entity_pattern_trans", "entity_inner_pattern_trans", "entity_latent_inner_pattern_trans"]:
                self.logger.log_stat("train/mac_disentangle_loss", mac_disentangle_loss.item(), t_env)
            if self.args.agent in ["entity_latent_inner_pattern_trans"]:
                self.logger.log_stat("train/mac_cmi_entropy_loss", mac_cmi_entropy_loss.item(), t_env)
                self.logger.log_stat("train/mac_cmi_kl_loss", mac_cmi_kl_loss.item(), t_env)
