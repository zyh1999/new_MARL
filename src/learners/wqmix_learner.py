...
from modules.mixers.wqmix import QMixerCentralFF, QMixerCentralAtten
import torch as th
...
class MAXQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if self.args.central_mixer in ["ff", "atten"]:
            if self.args.central_loss == 0:
                ...
                assert args.central_mac == "wqmix_mac"
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("train/qmix_loss", qmix_loss.item(), t_env)
            self.logger.log_stat("train/grad_norm", grad_norm.item(), t_env)
            self.logger.log_stat("train/mixer_norm", mixer_norm, t_env)
            self.logger.log_stat("train/agent_norm", agent_norm, t_env)
            ...
            self.logger.log_stat("train/central_loss", central_loss.item(), t_env)
            self.logger.log_stat("train/w_to_use", w_to_use, t_env)
