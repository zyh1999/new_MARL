...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        all_actions = actions.repeat(3, 1, 1, 1)
        all_chosen_action_qvals = th.gather(all_mac_out[:, :-1], dim=3, index=all_actions).squeeze(3)  # Remove the last dim

        mac_out, mac_out_W, mac_out_I = all_mac_out.chunk(3, dim=0)
        chosen_action_qvals, chosen_action_qvals_W, chosen_action_qvals_I = all_chosen_action_qvals.chunk(3, dim=0)
        chosen_action_qvals_imagine = th.cat([chosen_action_qvals_W, chosen_action_qvals_I], dim=2)
        ...
        if self.mixer is not None:
            ...
            if self.args.mixer == "vdn":
                chosen_action_qvals_imagine = self.mixer(chosen_action_qvals_imagine, inputs)
            else:
                chosen_action_qvals_imagine = self.mixer(chosen_action_qvals_imagine, inputs, imagine_groups=groups)
        ...
        # imagine loss
        td_error_imagine = (chosen_action_qvals_imagine - targets.detach())
        masked_td_error_imagine = td_error_imagine * mask
        loss_imagine = (masked_td_error_imagine ** 2).sum() / mask.sum()

        loss = (1 - self.args.loss_lambda) * loss + self.args.loss_lambda * loss_imagine
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            self.logger.log_stat("train/td_loss", loss.item(), t_env)
            self.logger.log_stat("train/im_loss", loss_imagine.item(), t_env)
